...
interface ConnectCostModalProps {
  ...
  costUSD: string;
  setCostUSD: (cost: string) => void;
  costINR: string;
  setCostINR: (cost: string) => void;
}

export default function ConnectCostModal({
  isOpen,
  onClose,
  onSubmit,
  editing,
    ...
    >
      <div className="bg-white w-full max-w-md rounded-xl shadow-xl p-6 animate-fadeIn">
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium mb-1">Cost per Connect (USD $)</label>
            <input
              type="number"
              min="0"
              step="0.01"
          ...
          <div>
            <label className="block text-sm font-medium mb-1">Cost per Connect (INR ₹)</label>
            <input
              type="number"
              min="0"
              step="0.01"
