...
export default function ViewStageDetailsModal({ isOpen, onClose, job }: any) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex justify-center items-center px-4">
      <div className="bg-white w-full max-w-md rounded-xl shadow-lg p-6 relative animate-fadeIn">
        <button className="absolute top-3 right-3 p-1 rounded-full hover:bg-gray-100" onClick={onClose}>
          <X size={20} />
        </button>
        <h2 className="text-2xl font-semibold text-gray-800 mb-4">{details.title}</h2>
        <div className="space-y-4">
          {details.fields.map((item: any, idx: number) => (
            <div key={idx}>
        ...
        <div className="mt-6 flex justify-end">
          <button className="px-4 py-2 bg-gray-200 rounded-lg text-gray-700 hover:bg-gray-300" onClick={onClose}>
            Close
          </button>
        </div>
      </div>
