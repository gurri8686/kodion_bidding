...
const MarkAsReplyModal = ({ isOpen, onClose, job, fetchAppliedJobs }: any) => {
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      await axios.put(
        ...
        { stage: "replied", date: dateTime.toISOString(), notes },
        { headers: { Authorization: `Bearer ${token}` }, withCredentials: true }
      );
      toast.success("Job marked as Replied!");
      fetchAppliedJobs();
      onClose();
    } catch (error) {
      console.error(error);
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 z-50">
      <div className="bg-white rounded-xl p-6 w-full max-w-md shadow-lg relative">
        ...
        <h2 className="text-xl font-semibold mb-4">Mark as Replied</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="relative" ref={pickerRef}>
            <label className="block text-sm font-medium text-gray-700 mb-1">Reply Date & Time</label>
            <div
            ...
            {isPickerOpen && (
              <div className="absolute mt-2 right-0 bg-white rounded-lg shadow-lg z-50 border w-[280px]">
                <DateRange
                  ...
                  onChange={(item: any) => setSelectedDate(item.selection.startDate)}
...
export default MarkAsReplyModal;
