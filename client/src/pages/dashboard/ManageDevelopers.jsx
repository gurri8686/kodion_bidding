...
const ManageDevelopers = () => {
  ...
  const fetchDevelopers = async () => {
    ...
    setError('');
    try {
      const res = await fetch(`${API_BASE}/api/get-all-developers`, {
        headers: {
  ...
  const handleAddDeveloper = async (data) => {
    ...
    setError('');
    try {
      const res = await fetch(`${API_BASE}/api/add-developer`, {
        method: 'POST',
        headers: {
  ...
  const handleUpdateDeveloper = async (data) => {
    setLoading(true);
    setError('');
    try {
      const res = await fetch(`${API_BASE}/api/edit-developer/${editingId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
  ...
  const handleDeleteDeveloper = async (developerId) => {
    ...
    setError('');
    try {
      const res = await fetch(`${API_BASE}/api/delete-developer/${developerId}`, {
        method: 'DELETE',
        credentials: 'include',
        headers: {
          'Content-Type': 'application/json',
  ...
  const DeveloperForm = ({ initialData = {}, onSave, onCancel, saveText = "Add Developer" }) => {
        ...
        <div className="p-6">
          ...
          {developers.length > 0 && (
            <div className="mt-6 text-center text-sm text-gray-500">
              Total Developers: <span className="font-medium text-gray-900">{developers.length}</span>
            </div>
          )} */}
          </div>
        </div>
