...
const Jobs = () => {
  ...
  const [jobId, setJobId] = useState(null);  // state to store selected job ID
  const jobsPerPage = 6;
  ...
  const fetchJobs = async (page = 1) => {
    ...
    try {
      const res = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/jobs/get-jobs`, {
        params,
  ...
  const handleApplyFilter = async () => {
    setIsApplying(true);
    try {
      await fetchJobs(1); // or current page
    } catch (error) {
  ...
  const handleClearFilter = () => {
    ...
    setCurrentPage(0); // if needed
  };


  // When user changes page
  const handlePageChange = ({ selected }) => {
    fetchJobs(selected + 1); // pagination component usually zero-based, API expects 1-based
  };
  ...
  const handleIgnore = async () => {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_API_BASE_URL}/api/jobs/ignore`,
        {
          reason: finalReason,
      ...
      if (response.status === 200) {
        ...
        closeModal();
        fetchJobs(currentPage + 1); // Re-fetch current page jobs
      }
    } catch (error) {
