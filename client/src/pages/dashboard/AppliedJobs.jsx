...
const AppliedJobs = () => {
  ...
  const applyDateFilter = (option) => {
    ...
    switch (option) {
      ...
      case "All": {
        // Set a very wide range for "All" - from 10 years ago to today
  ...
  const handleCustomRangeChange = (item) => {
  ...
  const fetchAppliedJobs = useCallback(
    async (page = 1) => {
      ...
      if (!isInitialLoad) {
        // For subsequent loads, no delay needed
  ...
  useEffect(() => {
    fetchAppliedJobs(1); // Reset to page 1 on filters
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {loading ? (
          ...
          <>
            ...
            {/* JOB DISPLAY - CARDS OR TABLE */}
            {appliedJobs.length > 0 && (
              <div className="relative">
                {/* Subtle loading overlay for non-initial loads */}
...
const AppliedJobsTable = ({ jobs, fetchAppliedJobs }) => {
  ...
  // Prevent body scroll when menu is open to avoid double scrollbar
  useEffect(() => {
    if (openMenuId) {
  ...
  const handleStageUpdate = (job, stage) => {
    setSelectedJob(job);
  ...
  return (
    <>
      ...
      {/* Portal-mounted actions menu (so it won't be clipped by table overflow) */}
