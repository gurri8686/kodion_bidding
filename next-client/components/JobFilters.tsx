...
import { DateRangePickerInput } from "../admin/components/DateRange";
...
const JobFilters = ({
  ...
  // 🔹 New date range state for react-date-range
  ...
  useEffect(() => {
    const fetchTechOptions = async () => {
      try {
        ...
        const endpoint =
          role === "admin"
            ? `${baseUrl}/api/jobs/technologies/names`
            : `${baseUrl}/api/jobs/technologies/${userId}`;
        ...
        if (role === "admin") {
          formatted = data.technologies.map((name) => ({
            value: name,
            label: name,
          }));
        } else {
        ...
        <div className="flex gap-3 items-center lg:flex-row flex-col ">
          <div className="flex-1">
            <JobSearch
              ...
              setSearchTerm={(val) => handleChange("searchTerm", val)}
