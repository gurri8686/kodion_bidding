...
const Sidebar = () => {
  ...
  const NavItem = ({ item }: { item: { label: string; path: string; icon: React.ReactNode } }) => {
    const isActive = item.label === "Dashboard" ? pathname === item.path : pathname.startsWith(item.path);
    return (
      <li key={item.path}>
        <Link
          href={item.path}
