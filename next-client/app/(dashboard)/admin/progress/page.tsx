...
const ProgressTracker = () => {
  ...
  const generateLastWeeks = (): Week[] => {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      const label = `${start.toDateString().slice(4, 10)} – ${end
        .toDateString()
        .slice(4, 10)}`;
  ...
  const [selectedWeek, setSelectedWeek] = useState<Week>(weekOptions[0]);
  ...
  const handleOpenModal = (mode: "create" | "edit") => {
    setModalMode(mode);
    setFormAmount(mode === "edit" ? String(targetAmount) : "");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      handleCloseModal();
      toast.success(
        `Target ${modalMode === "create" ? "created" : "updated"} successfully!`
      );
    } catch (error) {
  ...
  const progress =
    Number(targetAmount) > 0
      ? Math.round(Math.min((achievedAmount / Number(targetAmount)) * 100, 100))
      : 0;
          ...
          <>
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <select
                  ...
                  onChange={(e) =>
                    setSelectedWeek(weekOptions.find((w) => w.id == Number(e.target.value))!)
                  }
                ...
                <button
                  ...
                  className="flex items-center justify-center gap-2 px-6 py-3 bg-[#f76a00] text-white rounded-lg w-full sm:w-auto hover:bg-[#db6613]"
                >
            ...
            {!hasTarget ? (
              <div className="bg-white p-6 md:p-10 rounded-xl shadow text-center border">
                <Target size={50} className="mx-auto text-[#f76a00]" />
                ...
                <div className="bg-white p-4 md:p-6 rounded-xl shadow border mb-8">
                  ...
                  <div className="mt-6">
                    ...
                    <div className="bg-gray-200 w-full h-3 rounded-full overflow-hidden">
                      <div
                        className="bg-[#f76a00] h-3 rounded-full"
          ...
          <div className="fixed inset-0 bg-black bg-opacity-40 flex justify-center items-center p-4 z-50">
            <div className="bg-white w-full max-w-md rounded-xl p-4 md:p-6 shadow-xl">
              ...
              <form onSubmit={handleSubmit}>
                ...
                <label className="font-medium mb-1 block mt-4 text-sm md:text-base">
                  Achieved Amount (in $)
                </label>
                <input
                  type="number"
                ...
                <button
                  type="submit"
                  className="w-full mt-4 bg-[#f76a00] text-white py-2 md:py-3 rounded-lg text-sm md:text-base hover:bg-[#db6613]"
                >
